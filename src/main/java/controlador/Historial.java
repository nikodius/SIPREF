    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try (PrintWriter out = response.getWriter()) {
                ArrayList<HistorialDTO> listaHistorial = (ArrayList) facadePR.listarHistorial();
                request.setAttribute("listHistory", listaHistorial);
                request.getRequestDispatcher("history.jsp").forward(request, response);
            }
        } catch (Exception ex) {
            response.sendRedirect("history.jsp?er=" + ex.getMessage());
        }
    }
